...
## за покупку стала считать заказ, у которого есть время подтверждения оплаты заказа (order_approved_at) и статус delivered, потому что до доставки пользователь может отменить покупку
